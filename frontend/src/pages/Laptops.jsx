...
export default function Laptops() {
  ...
  return (
    <div>
      ...
      {/* Tableau */}
      <div style={{ background:'white', borderRadius:'12px', boxShadow:'0 1px 8px rgba(0,0,0,0.07)', overflow:'hidden' }}>
        {loading ? (
          ...
          <table style={{ width:'100%', borderCollapse:'collapse' }}>
            <thead>
              <tr style={{ background:'#f8fafc', borderBottom:'2px solid #e2e8f0' }}>
                {['ID','Marque','Modèle','Processeur','RAM','Stockage','État','Statut','Actions'].map(h => (
            ...
            <tbody>
              {laptops.map((l, i) => (
                <tr key={l.id_laptop} style={{ borderBottom:'1px solid #f1f5f9', background: i%2===0 ? 'white' : '#fafafa' }}>
                  <td style={{ padding:'0.85rem 1rem', fontSize:'0.85rem', color:'#64748b' }}>#{l.id_laptop}</td>
                  <td style={{ padding:'0.85rem 1rem', fontWeight:600, color:'#1e293b' }}>{l.marque}</td>
                  <td style={{ padding:'0.85rem 1rem', color:'#374151' }}>{l.modele}</td>
                  <td style={{ padding:'0.85rem 1rem', color:'#64748b', fontSize:'0.85rem' }}>{l.processeur}</td>
                  <td style={{ padding:'0.85rem 1rem', color:'#64748b' }}>{l.ram} Go</td>
                  <td style={{ padding:'0.85rem 1rem', color:'#64748b', fontSize:'0.85rem' }}>{l.stockage}</td>
                  <td style={{ padding:'0.85rem 1rem' }}><Badge value={l.etat} colors={{NEUF:'#22c55e',BON_ETAT:'#3b82f6',DEGRADE:'#f59e0b',HORS_SERVICE:'#ef4444'}} /></td>
                  <td style={{ padding:'0.85rem 1rem' }}><Badge value={l.statut} colors={STATUT_COLORS} /></td>
                  <td style={{ padding:'0.85rem 1rem' }}>
                    <button onClick={() => handleDelete(l.id_laptop)}
                      style={{ padding:'0.35rem 0.75rem', background:'#fee2e2', color:'#dc2626', border:'none', borderRadius:'6px', cursor:'pointer', fontSize:'0.8rem', fontWeight:600 }}>
                      Supprimer
                    </button>
                  </td>
                </tr>
        ...
        {!loading && laptops.length === 0 && (
          <div style={{ textAlign:'center', padding:'3rem', color:'#94a3b8' }}>Aucun laptop trouvé</div>
        )}
      </div>
    </div>
  );
